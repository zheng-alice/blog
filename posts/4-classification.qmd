...
Unlike [regression](3-regresison.qmd), it predicts features not continuous, but categorical and unordered.
...
This is a task easier said than done, as we need to carefully discard all unneccessary models, while making sure the important ones are retained.
...
Each neuron takes inputs from several others, aggregates them (plus a bias), thes passes through an activation function and onto the next layer.
...
We can also see some of the common digit pairs that confuse the model; $2$ and $7$, $8$ and $9$, $3$ and $5$.
