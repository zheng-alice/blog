...
* Computing how much a self-driving car should accelerate/deccelerate/turn (based on prior human data).
...
[^rv]: As hinted by this notation, $X$, $y$ and later $\epsilon$ can be thought of either as a [random vector/variables](1-probability.qmd) or matrix/vector samples of these.
...
The linear lines?
Turns out our brief description of [linear regression](#linear-regression) comes with a huge caveat: such regression is *linear* beause of the lineariarity of its *underlying models*, not necessarily because it fits straight lines.
...
Afterall, the transformed data will be exactly the linear data, for which an OLS fit prioritizes all residuals equally and thus works extremely well.
...
One particulary memorable example involved deriving the dynamics of an outbreak of an infectious disease with an SEIRS model and then using nonlinear regression to fit its parameters.
